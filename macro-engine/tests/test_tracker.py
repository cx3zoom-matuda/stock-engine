...
def test_kpis_calculation():
    ...
    # Hits & Misses:
    # BUY: Toyota (Hit), Softbank (Miss), INPEX (Pending - today)
    # AVOID: MUFG (Hit), NYK (Miss)
    # Evaluated active calls = BUY (Toyota, Softbank) + AVOID (MUFG, NYK) = 4
    # Hits = Toyota (1) + MUFG (1) = 2
    # Overall hit rate = 2 / 4 = 50.0%
    assert kpis["overall_hit_rate"] == 50.0
    
    # BUY Win Rate = 1 Hit / 2 Evaluated = 50.0%
    assert kpis["buy_hit_rate"] == 50.0
    
    # AVOID Win Rate = 1 Hit / 2 Evaluated = 50.0%
