...
st.markdown('<div class="title-text">📊 アルゴリズム＆計算ロジック解説</div>', unsafe_allow_html=True)
st.markdown('<div class="subtitle-text">本ツールがどのような数理ロジック、データ分析、評価マトリクスに基づいて自動スクリーニングと投資判断を行っているかを詳しく解説します。</div>', unsafe_allow_html=True)

# Tabs to organize information
tab_overview, tab_events, tab_matrix, tab_valuation, tab_decision = st.tabs([
    "🔍 1. 全体像・処理フロー",
    "🔔 2. マクロシグナル検出ロジック",
    "🏆 3. セクター影響度マトリクス",
    "⚖️ 4. 国別バリュエーション評価",
